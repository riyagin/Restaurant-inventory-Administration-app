import { useEffect, useState, useCallback } from 'react';
import { Link } from 'react-router-dom';
import { getAccounts, getAccountAdjustments, createAccountAdjustment, createAccountTransfer } from '../api';
import CurrencyInput from '../components/CurrencyInput';

const idr = (v) =>
  new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(v);

const TYPE_LABEL = { asset: 'Aset', liability: 'Kewajiban', equity: 'Ekuitas', revenue: 'Pendapatan', expense: 'Beban' };
const TYPE_COLOR = { asset: '#e8f5e9', liability: '#fce4ec', equity: '#e8eaf6', revenue: '#e6f9f0', expense: '#fff3e0' };
const TYPE_TEXT  = { asset: '#2e7d32', liability: '#880e4f', equity: '#283593', revenue: '#1b5e45', expense: '#e65100' };

const fmtDT = (d) => new Date(d).toLocaleString('id-ID', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });

function AccountBadge({ type, number, name }) {
  return (
    <div>
      <div style={{ fontWeight: 500, fontSize: '0.88rem' }}>{name}</div>
      <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.15rem', alignItems: 'center' }}>
        {number && (
          <span style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: '#4f8ef7' }}>{number}</span>
        )}
        <span style={{
          fontSize: '0.7rem', padding: '0.05rem 0.35rem', borderRadius: '3px',
          background: TYPE_COLOR[type] || '#eee',
          color: TYPE_TEXT[type] || '#555',
          fontWeight: 600,
        }}>
          {TYPE_LABEL[type] || type}
        </span>
      </div>
    </div>
  );
}

export default function AccountAdjustments() {
  const [accounts, setAccounts]           = useState([]);
  const [adjustments, setAdjustments]     = useState([]);
  const [filterAccount, setFilterAccount] = useState('');
  const [loading, setLoading]             = useState(false);
  const [error, setError]                 = useState('');
  const [success, setSuccess]             = useState('');
  const [mode, setMode]                   = useState('adjust'); // 'adjust' | 'transfer'

  const [form, setForm] = useState({
    account_id: '',
    sign: '+',
    amount: '',
    description: '',
  });

  const [transfer, setTransfer] = useState({
    from_account_id: '',
    to_account_id: '',
    amount: '',
    description: '',
  });

  const [submitting, setSubmitting] = useState(false);

  const loadAdjustments = useCallback(() => {
    setLoading(true);
    const params = filterAccount ? { account_id: filterAccount } : {};
    getAccountAdjustments(params)
      .then(r => setAdjustments(r.data))
      .finally(() => setLoading(false));
  }, [filterAccount]);

  const refreshAccounts = () => getAccounts().then(r => setAccounts(r.data));

  useEffect(() => { refreshAccounts(); }, []);
  useEffect(() => { loadAdjustments(); }, [loadAdjustments]);

  const groupedAccounts = accounts.reduce((g, a) => {
    if (!g[a.account_type]) g[a.account_type] = [];
    g[a.account_type].push(a);
    return g;
  }, {});

  const AccountSelect = ({ value, onChange, exclude, placeholder = '— Pilih akun —' }) => (
    <select value={value} onChange={onChange} required>
      <option value="">{placeholder}</option>
      {Object.entries(groupedAccounts).map(([type, accts]) => (
        <optgroup key={type} label={TYPE_LABEL[type] || type}>
          {accts.filter(a => a.id !== exclude).map(a => (
            <option key={a.id} value={a.id}>
              {a.account_number ? `${a.account_number} · ` : ''}{a.name}
            </option>
          ))}
        </optgroup>
      ))}
    </select>
  );

  const handleAdjustSubmit = async (e) => {
    e.preventDefault();
    setError(''); setSuccess('');
    const rawAmount = Number(form.amount);
    if (!rawAmount || rawAmount <= 0) { setError('Masukkan jumlah yang valid'); return; }
    const amount = form.sign === '-' ? -rawAmount : rawAmount;
    setSubmitting(true);
    try {
      await createAccountAdjustment({ account_id: form.account_id, amount, description: form.description });
      setSuccess('Penyesuaian berhasil disimpan.');
      setForm({ account_id: '', sign: '+', amount: '', description: '' });
      loadAdjustments();
      refreshAccounts();
    } catch (err) {
      setError(err.response?.data?.error || 'Terjadi kesalahan');
    } finally {
      setSubmitting(false);
    }
  };

  const handleTransferSubmit = async (e) => {
    e.preventDefault();
    setError(''); setSuccess('');
    const rawAmount = Number(transfer.amount);
    if (!rawAmount || rawAmount <= 0) { setError('Masukkan jumlah yang valid'); return; }
    if (transfer.from_account_id === transfer.to_account_id) {
      setError('Akun asal dan tujuan harus berbeda'); return;
    }
    setSubmitting(true);
    try {
      await createAccountTransfer({
        from_account_id: transfer.from_account_id,
        to_account_id:   transfer.to_account_id,
        amount:          rawAmount,
        description:     transfer.description,
      });
      setSuccess('Transfer berhasil disimpan.');
      setTransfer({ from_account_id: '', to_account_id: '', amount: '', description: '' });
      loadAdjustments();
      refreshAccounts();
    } catch (err) {
      setError(err.response?.data?.error || 'Terjadi kesalahan');
    } finally {
      setSubmitting(false);
    }
  };

  const selectedAccount = accounts.find(a => a.id === form.account_id);
  const fromAccount     = accounts.find(a => a.id === transfer.from_account_id);
  const toAccount       = accounts.find(a => a.id === transfer.to_account_id);

  // Group transfer pairs so each transfer shows as one row in history
  const historyRows = (() => {
    const seen = new Set();
    const rows = [];
    for (const adj of adjustments) {
      if (adj.transfer_id) {
        if (seen.has(adj.transfer_id)) continue;
        seen.add(adj.transfer_id);
        // Find the debit leg (negative amount = source)
        const debitLeg  = adjustments.find(a => a.transfer_id === adj.transfer_id && a.amount < 0);
        const creditLeg = adjustments.find(a => a.transfer_id === adj.transfer_id && a.amount > 0);
        rows.push({ type: 'transfer', id: adj.transfer_id, debitLeg, creditLeg, adj });
      } else {
        rows.push({ type: 'adjustment', id: adj.id, adj });
      }
    }
    return rows;
  })();

  const tabStyle = (active) => ({
    padding: '0.5rem 1.2rem',
    border: 'none',
    borderBottom: active ? '2px solid #4f8ef7' : '2px solid transparent',
    background: 'none',
    cursor: 'pointer',
    fontWeight: active ? 700 : 500,
    color: active ? '#4f8ef7' : '#666',
    fontSize: '0.9rem',
  });

  return (
    <>
      <div className="page-header">
        <h1>Jurnal Manual</h1>
        <Link to="/reports/financial" className="btn btn-secondary">← Laporan Keuangan</Link>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: '1.5rem', alignItems: 'start' }}>

        {/* ── Form ── */}
        <div className="card">
          {/* Mode tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid #eee', marginBottom: '1.25rem' }}>
            <button style={tabStyle(mode === 'adjust')} onClick={() => { setMode('adjust'); setError(''); setSuccess(''); }}>
              Penyesuaian
            </button>
            <button style={tabStyle(mode === 'transfer')} onClick={() => { setMode('transfer'); setError(''); setSuccess(''); }}>
              Transfer Antar Akun
            </button>
          </div>

          {error   && <div className="error-msg" style={{ marginBottom: '1rem' }}>{error}</div>}
          {success && <div style={{ background: '#e6f9f0', border: '1px solid #b2dfdb', borderRadius: '6px', padding: '0.75rem 1rem', marginBottom: '1rem', color: '#1b5e45', fontWeight: 500, fontSize: '0.88rem' }}>{success}</div>}

          {/* ── Adjustment form ── */}
          {mode === 'adjust' && (
            <form onSubmit={handleAdjustSubmit}>
              <div className="form-group">
                <label>Akun</label>
                <AccountSelect
                  value={form.account_id}
                  onChange={e => setForm(f => ({ ...f, account_id: e.target.value }))}
                />
              </div>

              {selectedAccount && (
                <div style={{ marginBottom: '1rem', padding: '0.6rem 0.9rem', borderRadius: '6px', background: TYPE_COLOR[selectedAccount.account_type] || '#f5f5f5', fontSize: '0.85rem' }}>
                  <span style={{ color: TYPE_TEXT[selectedAccount.account_type], fontWeight: 600 }}>
                    {TYPE_LABEL[selectedAccount.account_type]}
                  </span>
                  <span style={{ color: '#555', marginLeft: '0.75rem' }}>
                    Saldo saat ini: <strong>{idr(selectedAccount.balance)}</strong>
                  </span>
                </div>
              )}

              <div className="form-group">
                <label>Jenis &amp; Jumlah (Rp)</label>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <select
                    value={form.sign}
                    onChange={e => setForm(f => ({ ...f, sign: e.target.value }))}
                    style={{ width: 'auto', flexShrink: 0, fontWeight: 700, fontSize: '0.82rem', color: form.sign === '+' ? '#27ae60' : '#e74c3c' }}
                  >
                    <option value="+">+ Tambah</option>
                    <option value="-">− Kurangi</option>
                  </select>
                  <CurrencyInput
                    value={form.amount}
                    onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
                    placeholder="0"
                    min="1"
                    required
                    style={{ flex: 1 }}
                  />
                </div>
                {form.amount && selectedAccount && (
                  <small style={{ color: '#888', marginTop: '0.3rem', display: 'block' }}>
                    Saldo baru: <strong style={{ color: form.sign === '+' ? '#27ae60' : '#e74c3c' }}>
                      {idr(selectedAccount.balance + (form.sign === '-' ? -1 : 1) * Number(form.amount))}
                    </strong>
                  </small>
                )}
              </div>

              <div className="form-group">
                <label>Keterangan / Alasan</label>
                <input
                  type="text"
                  value={form.description}
                  onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  placeholder="Contoh: Biaya promosi bulan Januari..."
                  required
                />
              </div>

              <button type="submit" className="btn btn-primary" disabled={submitting} style={{ width: '100%', justifyContent: 'center' }}>
                {submitting ? 'Menyimpan...' : 'Simpan Penyesuaian'}
              </button>
            </form>
          )}

          {/* ── Transfer form ── */}
          {mode === 'transfer' && (
            <form onSubmit={handleTransferSubmit}>
              <div className="form-group">
                <label>Dari Akun (Sumber)</label>
                <AccountSelect
                  value={transfer.from_account_id}
                  onChange={e => setTransfer(f => ({ ...f, from_account_id: e.target.value }))}
                  exclude={transfer.to_account_id}
                  placeholder="— Pilih akun asal —"
                />
                {fromAccount && (
                  <div style={{ marginTop: '0.4rem', padding: '0.5rem 0.75rem', borderRadius: '6px', background: TYPE_COLOR[fromAccount.account_type] || '#f5f5f5', fontSize: '0.83rem' }}>
                    <span style={{ color: TYPE_TEXT[fromAccount.account_type], fontWeight: 600 }}>{TYPE_LABEL[fromAccount.account_type]}</span>
                    <span style={{ color: '#555', marginLeft: '0.75rem' }}>Saldo: <strong>{idr(fromAccount.balance)}</strong></span>
                  </div>
                )}
              </div>

              <div style={{ textAlign: 'center', fontSize: '1.2rem', color: '#aaa', margin: '0.25rem 0' }}>↓</div>

              <div className="form-group">
                <label>Ke Akun (Tujuan)</label>
                <AccountSelect
                  value={transfer.to_account_id}
                  onChange={e => setTransfer(f => ({ ...f, to_account_id: e.target.value }))}
                  exclude={transfer.from_account_id}
                  placeholder="— Pilih akun tujuan —"
                />
                {toAccount && (
                  <div style={{ marginTop: '0.4rem', padding: '0.5rem 0.75rem', borderRadius: '6px', background: TYPE_COLOR[toAccount.account_type] || '#f5f5f5', fontSize: '0.83rem' }}>
                    <span style={{ color: TYPE_TEXT[toAccount.account_type], fontWeight: 600 }}>{TYPE_LABEL[toAccount.account_type]}</span>
                    <span style={{ color: '#555', marginLeft: '0.75rem' }}>Saldo: <strong>{idr(toAccount.balance)}</strong></span>
                  </div>
                )}
              </div>

              <div className="form-group">
                <label>Jumlah Transfer (Rp)</label>
                <CurrencyInput
                  value={transfer.amount}
                  onChange={e => setTransfer(f => ({ ...f, amount: e.target.value }))}
                  placeholder="0"
                  min="1"
                  required
                />
                {transfer.amount && fromAccount && toAccount && (
                  <small style={{ color: '#888', marginTop: '0.3rem', display: 'block' }}>
                    {fromAccount.name}: <strong style={{ color: '#e74c3c' }}>{idr(fromAccount.balance - Number(transfer.amount))}</strong>
                    {' · '}
                    {toAccount.name}: <strong style={{ color: '#27ae60' }}>{idr(toAccount.balance + Number(transfer.amount))}</strong>
                  </small>
                )}
              </div>

              <div className="form-group">
                <label>Keterangan / Alasan</label>
                <input
                  type="text"
                  value={transfer.description}
                  onChange={e => setTransfer(f => ({ ...f, description: e.target.value }))}
                  placeholder="Contoh: Pindah dana ke kas cabang..."
                  required
                />
              </div>

              <button type="submit" className="btn btn-primary" disabled={submitting} style={{ width: '100%', justifyContent: 'center' }}>
                {submitting ? 'Memproses...' : 'Simpan Transfer'}
              </button>
            </form>
          )}
        </div>

        {/* ── History ── */}
        <div className="card">
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <h2>Riwayat {adjustments.length > 0 && `(${historyRows.length})`}</h2>
            <div className="filters">
              <select
                value={filterAccount}
                onChange={e => setFilterAccount(e.target.value)}
                style={{ minWidth: '220px' }}
              >
                <option value="">Semua Akun</option>
                {Object.entries(groupedAccounts).map(([type, accts]) => (
                  <optgroup key={type} label={TYPE_LABEL[type] || type}>
                    {accts.map(a => (
                      <option key={a.id} value={a.id}>
                        {a.account_number ? `${a.account_number} · ` : ''}{a.name}
                      </option>
                    ))}
                  </optgroup>
                ))}
              </select>
              {filterAccount && (
                <button className="btn btn-secondary btn-sm" onClick={() => setFilterAccount('')}>Bersihkan</button>
              )}
            </div>
          </div>

          <table>
            <thead>
              <tr>
                <th>Akun</th>
                <th>Keterangan</th>
                <th style={{ textAlign: 'right' }}>Jumlah</th>
                <th>Oleh</th>
                <th>Waktu</th>
              </tr>
            </thead>
            <tbody>
              {historyRows.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', color: '#999', padding: '2rem' }}>
                  {loading ? 'Memuat...' : 'Belum ada penyesuaian'}
                </td></tr>
              ) : historyRows.map(row => {
                if (row.type === 'transfer') {
                  const { debitLeg, creditLeg, adj } = row;
                  return (
                    <tr key={row.id} style={{ background: '#f0f4ff' }}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.2rem' }}>
                          <span style={{ fontSize: '0.7rem', padding: '0.05rem 0.4rem', borderRadius: '3px', background: '#dde8ff', color: '#283593', fontWeight: 700, letterSpacing: '0.03em' }}>TRANSFER</span>
                        </div>
                        {debitLeg && (
                          <div style={{ fontSize: '0.82rem', color: '#e74c3c', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                            <span>−</span>
                            <AccountBadge type={debitLeg.account_type} number={debitLeg.account_number} name={debitLeg.account_name} />
                          </div>
                        )}
                        {creditLeg && (
                          <div style={{ fontSize: '0.82rem', color: '#27ae60', display: 'flex', alignItems: 'center', gap: '0.3rem', marginTop: '0.2rem' }}>
                            <span>+</span>
                            <AccountBadge type={creditLeg.account_type} number={creditLeg.account_number} name={creditLeg.account_name} />
                          </div>
                        )}
                      </td>
                      <td style={{ color: '#555', fontSize: '0.88rem', maxWidth: '200px' }}>{adj.description}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: '#4f8ef7', whiteSpace: 'nowrap' }}>
                        {idr(creditLeg ? Math.abs(creditLeg.amount) : Math.abs(adj.amount))}
                      </td>
                      <td style={{ color: '#888', fontSize: '0.82rem' }}>{adj.created_by_name || '—'}</td>
                      <td style={{ color: '#888', fontSize: '0.82rem', whiteSpace: 'nowrap' }}>{fmtDT(adj.created_at)}</td>
                    </tr>
                  );
                }
                const { adj } = row;
                return (
                  <tr key={row.id}>
                    <td>
                      <AccountBadge type={adj.account_type} number={adj.account_number} name={adj.account_name} />
                    </td>
                    <td style={{ color: '#555', fontSize: '0.88rem', maxWidth: '220px' }}>{adj.description}</td>
                    <td style={{ textAlign: 'right', fontWeight: 700, color: adj.amount > 0 ? '#27ae60' : '#e74c3c', whiteSpace: 'nowrap' }}>
                      {adj.amount > 0 ? '+' : ''}{idr(adj.amount)}
                    </td>
                    <td style={{ color: '#888', fontSize: '0.82rem' }}>{adj.created_by_name || '—'}</td>
                    <td style={{ color: '#888', fontSize: '0.82rem', whiteSpace: 'nowrap' }}>{fmtDT(adj.created_at)}</td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      </div>
    </>
  );
}
