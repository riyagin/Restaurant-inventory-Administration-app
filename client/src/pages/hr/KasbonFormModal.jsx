...
export default function KasbonFormModal({ onClose, onSaved, presetEmployeeId }) {
  ...
  const submit = async () => {
    ...
    if (split) {
      if (splitSum !== Math.round(total)) { setError('Total cicilan harus sama dengan jumlah kasbon.'); return; }
      if (rows[0].due_month === rows[1].due_month) { setError('Bulan cicilan tidak boleh sama.'); return; }
    ...
    <div style={overlay} onClick={saving ? undefined : onClose}>
      <div className="card" style={modal} onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <select value={form.employee_id} onChange={e => setForm({ ...form, employee_id: e.target.value })} disabled={!!presetEmployeeId}>
