...
- **store_manager** is a limited role for branch staff who help visiting employees record attendance. It can authenticate to the web app but is scoped to attendance only via `RequireAttendanceAccess`: `GET /api/hr/attendance` (List, unguarded) and `PUT /api/hr/attendance/{id}` (record entry/correction), plus the read-only employee/position lookups any authenticated user can reach. Every other `/api/hr/*` endpoint and all non-HR routes return 403, since they require admin/manager. Attendance **configuration** (work-schedules, holidays, devices, reconcile, fingerprint-import) remains admin/manager only.
