...
// List — GET /api/hr/overtime?month=YYYY-MM&employee_id=UUID
...
SELECT o.id, o.employee_id, e.full_name AS employee_name, e.employee_code,
       o.date, o.hours::float8, o.reason, u.username AS created_by_username, o.created_at
...
WHERE true`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&dateVal, &item.Hours, &item.Reason, &item.CreatedByUsername, &createdAt,
		); err != nil {
...
// Create — POST /api/hr/overtime (admin only)
func (h *OvertimeHandler) Create(w http.ResponseWriter, r *http.Request) {
	if middleware.RoleFromCtx(r.Context()) != "admin" {
		respondError(w, http.StatusForbidden, "hanya admin yang dapat menambahkan permintaan lembur")
		return
	}
